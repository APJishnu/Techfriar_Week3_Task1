...
const HeroSection: React.FC = () => {
  ...
  return (
    ...
    {/* section starting mobile booking  */}
    <section className={styles.mobileBookingSection}>
      <div className={styles.mobileBookingContainer}>
        <div className={styles.mobileBookingButtons}>
          <button
            id="bookCarBtn"
            className={`${styles.mobileBookingButton} ${activeButton === 'bookCarBtn' ? styles.activeButton : ''}`}
            onClick={toggleTripOptions}
          >
            Book a Car
          </button>
          <button
            id="quickBookBtn"
            className={`${styles.mobileBookingButton} ${styles.outline}`}
          >
            Quick Book
          </button>
        </div>
        ...
        {showTripOptions && (
          <div id="tripOptions" className={styles.tripOptionsContainer}>
            <div className={styles.selectorOptions}>
              <button
                className={`${styles.selectorOption} ${activeButton === 'samePickUpBtn' ? styles.activeButton : ''}`}
                id="samePickUpBtn"
                onClick={togglePickupOptions}
              >
                Same as Pick-Up <span className={styles.line}></span>
              </button>
              <button
                className={`${styles.selectorOption} ${activeButton === 'differentDropOffBtn' ? styles.activeButton : ''}`}
                id="differentDropOffBtn"
                onClick={toggleDropoffOptions}
              >
                Different Drop-Off <span className={styles.line}></span>
              </button>
            ...
            {showDropoffOptions && (
              <div id="dropOffOptions" className={styles.dropdownOptions}>
                <select>
                  <option value="location3">Pick-Up Location</option>
                  <option value="location4">Location 2</option>
                </select>
                <select>
                  <option value="location5">Drop-Off Location</option>
                  <option value="location6">Location 2</option>
                </select>
                <input type="datetime-local" placeholder="Pick-Up Date & Time" />
                <input type="datetime-local" placeholder="Drop-Off Date & Time" />
              </div>
